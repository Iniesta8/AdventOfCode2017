...
static int input[NR_LINES][NR_ROWS] =
...
static int calc_line_difference(int line[], int len) {
  int min = line[0];
  int max = line[0];
  int i = 0;
...
int main(void) {
  ...
  for (i = 0; i < NR_LINES; i++) {
    sum_of_differences += calc_line_difference(input[i], NR_ROWS);
  }

  printf("\nSum of all differences: %d\n\n", sum_of_differences);
